...
type DefaultTabInput = number | keyof PricingData;

interface PricingComponentProps {
  /**
   * If provided, the component will lock to this tab and hide/disable
   * tab-switching UI. Accepts:
   *  - number: 1 => ebook, 2 => cover, 3 => publishing, 4 => video, 5 => marketing
   *  - string: one of 'ebook'|'cover'|'publishing'|'video'|'marketing'
   */
  defaultTab?: DefaultTabInput;
}

const PricingComponent: React.FC<PricingComponentProps> = ({ defaultTab }) => {
  ...
  // Helper to resolve numeric or string defaultTab into a tab id
  const resolveTabId = (input?: DefaultTabInput): keyof PricingData => {
    if (!input) return "ebook"; // fallback default
    if (typeof input === "string") {
      // validate
      if (tabs.find((t) => t.id === input)) return input;
      return "ebook";
    }
    // numeric mapping: 1..5 -> tabs order
    const idx = Math.max(1, Math.min(input, tabs.length)) - 1;
    return tabs[idx].id;
  };

  // Determine initial active tab from defaultTab (if provided) or fallback to 'ebook'
  const initialTab = resolveTabId(defaultTab);

  // If defaultTab provided, we lock to that tab id and hide tab switching.
  const [activeTab, setActiveTab] = useState<keyof PricingData>(initialTab);

  // When defaultTab prop changes, update activeTab accordingly
  useEffect(() => {
    setActiveTab(resolveTabId(defaultTab));
    ...
    >
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "50px" }}>
          ...
          >
            With Our Reasonably Priced Packages, Become a Successful Writer and
            ...
            <div
              ...
              onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => {
                ...
                e.currentTarget.style.boxShadow =
                  "0 12px 48px rgba(54, 74, 82, 0.25)";
              }}
        ...
        >
          ...
          >
            ...
            we will help you to choose your best pricing plan.
          </h6>
        </div>
      </div>
    </div>
